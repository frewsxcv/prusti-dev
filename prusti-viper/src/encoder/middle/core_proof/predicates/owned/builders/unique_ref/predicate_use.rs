    ...
    G: WithLifetimes + WithConstArguments,
{
    ...
    snapshots: Option<(vir_low::Expression, vir_low::Expression)>,
    ...
    G: WithLifetimes + WithConstArguments,
{
    ...
    pub(in super::super::super::super::super) fn build(
        ...
        let expression = if let Some((current_snapshot, final_snapshot)) = self.snapshots.take() {
    ...
    pub(in super::super::super::super::super) fn add_snapshot_arguments(
        &mut self,
        current_snapshot: vir_low::Expression,
        final_snapshot: vir_low::Expression,
        ...
        if config::use_snapshot_parameters_in_predicates() {
            ...
            self.snapshots = Some((current_snapshot, final_snapshot));
        }
        Ok(())
    }
