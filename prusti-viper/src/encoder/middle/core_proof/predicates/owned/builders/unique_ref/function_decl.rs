...
impl<'l, 'p, 'v, 'tcx> UniqueRefSnapFunctionBuilder<'l, 'p, 'v, 'tcx> {
    ...
    pub(in super::super::super) fn build(
        self,
    ) -> SpannedEncodingResult<(vir_low::FunctionDecl, vir_low::FunctionDecl)> {
    ...
    // ) -> SpannedEncodingResult<()> {
    //     let predicate = self.predcondition_predicate()?;
    ...
    fn predcondition_predicate(&mut self) -> SpannedEncodingResult<vir_low::Expression> {
